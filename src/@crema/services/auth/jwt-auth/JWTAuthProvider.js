...
const JWTAuthAuthProvider = ({children}) => {
  ...
  jwtAxios.interceptors.response.use(
      (res) => res,
      async (err) => {
        const getRefToken = localStorage.getItem('emRefToken')
        ...
        if (err.response.status === 401 && err.response.config.method !== "get" && getRefToken) {
          try {
            ...
            localStorage.setItem('emtoken', newToken.access)
  ...
  const signInUser = async ({phone, password}) => {
    ...
    try {
      ...
      localStorage.setItem('emtoken', data.access);
      localStorage.setItem('emRefToken', data.refresh);
